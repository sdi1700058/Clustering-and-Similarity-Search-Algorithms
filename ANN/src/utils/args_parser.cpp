...
Args parse_args(int argc, char** argv) {
    ...
    args.metric       = mp.count("-metric") ? mp["-metric"] : // distance METRIC
                        get_or_prompt("-metric", "Enter distance metric (l1/l2/cosine)", "l2");
    ...
    info << "\n[INFO] Configuration Summary:\n"
            ...
            << "  Params:    N=" << args.N << ", R=" << args.R 
            << ", Range=" << (args.range ? "Enabled" : "Disabled") << "\n";

    if (args.algo == "lsh") {
        info << "  LSH Args:  k=" << args.k << ", L=" << args.L << ", w=" << args.w << "\n";
    } else if (args.algo == "hypercube") {
        info << "  Cube Args: kproj=" << args.kproj << ", M=" << args.M << ", probes=" << args.probes << ", w=" << args.w << "\n";
    } else if (args.algo == "ivfflat" || args.algo == "ivfpq") {
        info << "  IVF Args:  kclusters=" << args.kclusters << ", nprobe=" << args.nprobe << "\n";
        if (args.algo == "ivfpq") info << "  PQ Args:   M=" << args.pq_M << ", nbits=" << args.pq_nbits << "\n";
    }
