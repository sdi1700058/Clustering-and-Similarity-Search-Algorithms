...
void IVFFlatSearch::build_index(const std::vector<Vector>& dataset) {
    ...
    // Initialize using Lloyd's k-means
    random_subset();
...
SearchResult IVFFlatSearch::search(const Vector& query, const Params& params, int query_id) const {
    ...
    // Select the top 'nprobes' closest centroids
    ...
    // Retrieve global top-N neighbors

    if (!b.empty()) {
        
        size_t sort_end = std::min((size_t)params.N, b.size());
        std::partial_sort(b.begin(), b.begin() + sort_end, b.end(), [](const auto& a, const auto& b) { return a.second < b.second; });
        
        int topK = static_cast<int>(sort_end);
...
void IVFFlatSearch::random_subset() {
    ...
    // Ensure we have enough points for kclusters centroids
    size_t subset_size = std::max((size_t)p.kclusters, (size_t)std::sqrt(n_points));
    if (subset_size > (size_t)n_points) subset_size = (size_t)n_points;
